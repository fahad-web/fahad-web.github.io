...
const managerData = ({ data }) => {
  ...
  const tourguid = router.query.id;
  ...
  const deleteData = async () => {
    try {
      ...
      const response = await axios.delete(process.env.NEXT_PUBLIC_MAIN_URL + '/delete/tourguid/' + tourguid, { withCredentials: true });
  ...
  const loadAllData = async () => {
    try {
      const respons = await axios.get(process.env.NEXT_PUBLIC_MAIN_URL + '/gettourguid/' + tourguid);
            ...
            <div class="w-full max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
              ...
              <div class="flex flex-col items-center pb-10">
                ...
                <div class="flex mt-4 space-x-3 md:mt-6">
                  ...
                  <Link href="/tourguid/update_tourguid" class="inline-flex items-center px-4 py-2 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Update</Link>
